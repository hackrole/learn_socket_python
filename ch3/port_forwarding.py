...
class Receiveer(asyncore.dispatcher):
...
class Sender(asyncore.dispatcher):
    ...
    def __init__(self, receive, remoteaddr, remoteport):
        asyncore.dispatcher.__init__(self)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--remote-port", action="store", dest="remote_port",
                        type=int, default=80)

    given_args = parser.parse_args()
    local_host, remote_host = given_args.local_host, given_args.remote_host
    local_port, remtoe_port = given_args.local_port, given_args.remote_port
